...
public class SQLWrapper {
			...
			for(int i=0;i<methods.length;i++){
			  if(methods[i].getName().startsWith("get")){
				  try {
                      ...
                      fields.add(this.fieldNameWrapper(methods[i].getName(),meta));
	...
	public SQLWrapper delete(EntityObject Model){
		...
		for(int i=0;i<methods.length;i++){
			  if(methods[i].getName().startsWith("get")){
				  try {
                      String fieldName = fieldNameWrapper(methods[i].getName(),meta);
			...
			if(methods[i].getName().startsWith("get")){
				  try {
					  ...
					  if(null!=tempValue&&!tempValue.equals("")){
						  fields.add(fieldNameWrapper(methods[i].getName(),meta));
	...
	public SQLWrapper update(EntityObject Model){
		...
		if(Model!=null){
			...
			for(int i=0;i<methods.length;i++){
			  if(methods[i].getName().startsWith("get")){
				  try {
					  fieldName = fieldNameWrapper(methods[i].getName(),meta);
					  Object value = methods[i].invoke(Model, (Object[])null);  // 值
					  //判断是否是主键
					  if(set.contains(fieldName)){
						  keys.add(fieldName);
	...
	*/
    ...
    //获取field在数据库中的真实字段名字
    protected String fieldNameWrapper(String fieldName,AnnotatedModel meta){
