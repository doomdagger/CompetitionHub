...
public class ExcelBinderTool<T> {
	/**
	 * 
	 */
	...
	private List<Map<String,Field>> sortBinder = new ArrayList<Map<String,Field>>();
	...
	public ExcelBinderTool(Class<? extends T> cls){
		this.cls = cls;
	}
	 ...
	 * @return
	 */
	/*public ExcelBinderTool<T> bind(String fieldName, String printName) {
		try {
	...
	public ExcelBinderTool<T> bind(String fieldName,String printName){
		try {
			Map<String,Field> column = new HashMap<String,Field>();
			column.put(printName, cls.getDeclaredField(fieldName));
			sortBinder.add(column);
		} catch (Exception e) {
			e.printStackTrace();
			System.err.println("Bind "+fieldName+" to "+printName+" failed.\n"+e.getMessage());
		}
	...
	public void genExcel(List<T> data, OutputStream out) throws Exception{
		...
		for(Map<String,Field> column : sortBinder){
			int tempRowIndex = localStartRow;
			for(Map.Entry<String, Field> entry : column.entrySet()){
				sheet.addCell(new Label(tempColumnIndex, tempRowIndex++, (String) entry.getKey(), getHeaderCellFormat()));
				for(T t : data){
